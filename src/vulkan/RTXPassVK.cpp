...
RTXPassVK::RTXPassVK(
    ...
    m_in_position_depth_attachment(i_position_depth_attachment),
    m_out_rtx_attachment(o_rtx_attachment)
{
...
void RTXPassVK::createFbo()
{
    ...
    for( size_t i = 0; i < m_fbos.size(); i++ )
    {
        ...
        attachments[ 0 ] = m_out_rtx_attachment.m_image_view;     // SSAO attachment
...
void RTXPassVK::createRenderPass()
{
    ...
    // Color  attachment
    attachments[ 0 ].format = m_out_rtx_attachment.m_format;
    ...
    VkAttachmentReference rtx_reference = {};
    rtx_reference.attachment = 0;
    rtx_reference.layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;

    std::array<VkAttachmentReference, 1> attachments_references = { rtx_reference };
...
{
    std::array<VkDescriptorSetLayoutBinding, 3> rtx_bindings = {};

    // Binding 0: Uniform buffer for per-frame data
    rtx_bindings[ 0 ].binding = 0;
    rtx_bindings[ 0 ].descriptorCount = 1;
    rtx_bindings[ 0 ].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    rtx_bindings[ 0 ].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;

    // Binding 1: Combined image sampler for position-depth attachment
    rtx_bindings[ 1 ].binding = 1;
    rtx_bindings[ 1 ].descriptorCount = 1;
    rtx_bindings[ 1 ].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    rtx_bindings[ 1 ].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;

    // Binding 2: Acceleration structure for ray tracing
    rtx_bindings[ 2 ] = {};
    rtx_bindings[ 2 ].binding = 2;
    rtx_bindings[ 2 ].descriptorCount = 1;
    rtx_bindings[ 2 ].descriptorType = VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR;
    rtx_bindings[ 2 ].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;

    VkDescriptorSetLayoutCreateInfo set_rtx_info = {};
    set_rtx_info.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
    set_rtx_info.pNext = nullptr;
    set_rtx_info.bindingCount = rtx_bindings.size();
    set_rtx_info.pBindings = rtx_bindings.data();

    if (VK_SUCCESS != vkCreateDescriptorSetLayout(m_runtime.m_renderer->getDevice()->getLogicalDevice(), &set_rtx_info, nullptr, &m_descriptor_set_layout))
    {
...
void RTXPassVK::createDescriptors()
{
    ...
    for (uint32_t id = 0; id < m_runtime.m_renderer->getWindow().getImageCount(); id++)
    {
        ...
        alloc_ssao_info.pSetLayouts = &m_descriptor_set_layout;
        ...
        pos_info.sampler = m_in_position_depth_attachment.m_sampler;
        pos_info.imageView = m_in_position_depth_attachment.m_image_view;
