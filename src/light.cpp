...
Matrix4f MiniEngine::Light::getLightSpaceMatrix(std::shared_ptr<Light> i_light, float nearPlane, float farPlane, Camera& i_camera)
{
    ...
    case LightType::Point:
        ...
        Matrix4f projectionMatrix =
            glm::perspective(glm::radians(i_light->m_data.m_fov), 1.0f, nearPlane, farPlane);
    ...
    /*float dist = i_camera.getFarPlane() - i_camera.getNearPlane();
	float nearPlane = i_light->m_data.m_near + nearSplit * dist;
    float farPlane = i_light->m_data.m_near + farSplit * dist;*/

        // Lambda for getting FRUSTRUM corners
        auto getFrustumCornersWorldSpace = [](Matrix4f projview) {
            const auto inv = glm::inverse(projview);
            ...
            for (auto& corner : frustumCorners) {
                Vector4f pt = inv * corner;
                corner = pt / pt.w; // División de perspectiva
            }
            /*std::vector<Vector4f> frustumCorners;
            frustumCorners.reserve(8);
        ...
        /*auto corners = getFrustumCornersWorldSpace(i_camera.getProjection() * i_camera.getView());

        for (uint32_t i = 0; i < 4; i++) {
			Vector3f dist = corners[i + 4] - corners[i];
            corners[i + 4] = corners[i] + Vector4f(dist * farPlane, 1.f);
            corners[i] = corners[i] + Vector4f(dist * nearPlane, 1.f);
        }*/
        float cameraNear = i_camera.getNearPlane();
        float cameraFar = i_camera.getFarPlane();
        float depthRange = cameraFar - cameraNear;

        auto corners = getFrustumCornersWorldSpace(i_camera.getProjection() * i_camera.getView());

        for (uint32_t i = 0; i < 4; i++) {
            Vector4f ray = corners[i + 4] - corners[i];

            // Calcular el porcentaje (0.0 a 1.0) a lo largo del rayo
            float nearRatio = (nearPlane - cameraNear) / depthRange;
            float farRatio = (farPlane - cameraNear) / depthRange;

            // Guardar la esquina original para no sobreescribirla antes del cálculo far
            Vector4f originalNearCorner = corners[i];

            // Aplicar la interpolación correctamente
            corners[i] = originalNearCorner + (ray * nearRatio);
            corners[i + 4] = originalNearCorner + (ray * farRatio);
        }
